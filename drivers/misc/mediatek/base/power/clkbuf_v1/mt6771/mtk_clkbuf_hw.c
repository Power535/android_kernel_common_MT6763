...
static void clk_buf_get_bblpm_en(void)
{
	...
	clk_buf_pr_info("%s: sel ctrl_dbg7: rg_auxout=0x%x\n",
		__func__, rg_auxout);

	xo_bb_lpm_en_stat = (rg_auxout & (0x1 << 0)) >> 0;

	clk_buf_pr_info("%s: bblpm %d\n", __func__, xo_bb_lpm_en_stat);
}
...
static ssize_t clk_buf_show_status_info_internal(char *buf)
{
	...
	len += snprintf(buf+len, CLKBUF_STATUS_INFO_SIZE-len, "pmic_drv_curr_vals=%d %d %d %d %d %d %d\n",
			...
			PMIC_CLK_BUF7_DRIVING_CURR);
	len += snprintf(buf+len, CLKBUF_STATUS_INFO_SIZE-len, "clkbuf_drv_curr_auxout=%d %d %d %d %d %d %d\n",
		     clkbuf_drv_curr_auxout[XO_SOC],
		     clkbuf_drv_curr_auxout[XO_WCN],
		     clkbuf_drv_curr_auxout[XO_NFC],
		     clkbuf_drv_curr_auxout[XO_CEL],
	...
	len += snprintf(buf+len, CLKBUF_STATUS_INFO_SIZE-len, "clkbuf_ctrl_stat=0x%x, pwrap_dcxo_en_flag=0x%x\n",
	...
	pmic_read_interface_nolock(PMIC_RG_SRCLKEN_IN3_EN_ADDR, &pmic_cw00,
			    PMIC_REG_MASK, PMIC_REG_SHIFT);
	len += snprintf(buf+len, CLKBUF_STATUS_INFO_SIZE-len, "SRCLKEN_IN3_EN(srclken_conn)=0x%x\n", pmic_cw00);

	len += snprintf(buf+len, CLKBUF_STATUS_INFO_SIZE-len, "DCXO_CONN_ADR0/WDATA0/ADR1/WDATA1=0x%x %x %x %x\n",
		     clkbuf_readl(DCXO_CONN_ADR0),
		     clkbuf_readl(DCXO_CONN_WDATA0),
		     clkbuf_readl(DCXO_CONN_ADR1),
		     clkbuf_readl(DCXO_CONN_WDATA1));
	len += snprintf(buf+len, CLKBUF_STATUS_INFO_SIZE-len, "DCXO_NFC_ADR0/WDATA0/ADR1/WDATA1/EN=0x%x %x %x %x %x\n",
	...
	len += snprintf(buf+len, CLKBUF_STATUS_INFO_SIZE-len, "bblpm_switch=%u, bblpm_cnt=%u, bblpm_cond=0x%x\n",
	...
	struct kobj_attribute *attr, const char *buf, size_t count)
{
	...
	if ((kstrtouint(buf, 10, &onoff))) {
		clk_buf_pr_err("bblpm input error\n");
		return -EPERM;
	}
	clk_buf_pr_info("bblpm input = %d\n", onoff);
	if (onoff == 1)
