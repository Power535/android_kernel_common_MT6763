...
#define UNGYROHUB_PR_ERR(fmt, args...)    pr_err(UNGYROHUB_TAG"%s %d : "fmt, __func__, __LINE__, ##args)
...
static int uncali_gyrohub_local_init(void)
{
	...
	if (err) {
		UNGYROHUB_PR_ERR("register uncali_gyro control path err\n");
		goto exit;
	}
	...
	if (err < 0) {
		UNGYROHUB_PR_ERR("SCP_sensorHub_data_registration failed\n");
		goto exit;
	}
	return 0;
exit:
