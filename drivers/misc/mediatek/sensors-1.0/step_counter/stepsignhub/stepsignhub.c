...
#define STEP_CDS_FUN(f)               pr_debug(STEP_CDS_TAG"%s\n", __func__)
#define STEP_CDS_PR_ERR(fmt, args...)    pr_err(STEP_CDS_TAG"%s %d : "fmt, __func__, __LINE__, ##args)
#define STEP_CDS_LOG(fmt, args...)    pr_debug(STEP_CDS_TAG fmt, ##args)
...
static ssize_t store_trace_value(struct device_driver *ddri, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "0x%x", &trace) == 1) {
		...
		STEP_CDS_PR_ERR("invalid content: '%s', length = %zu\n", buf, count);
		return 0;
	}
	return count;
}
...
static int step_chub_create_attr(struct device_driver *driver)
{
	...
	for (idx = 0; idx < num; idx++) {
		...
		if (err != 0) {
			STEP_CDS_PR_ERR("driver_create_file (%s) = %d\n",
...
static int step_counter_get_data(uint32_t *counter, int *status)
{
	...
	if (err < 0) {
		STEP_CDS_PR_ERR("sensor_get_data_from_hub fail!!\n");
		return -1;
	}
...
static int floor_counter_get_data(uint32_t *counter, int *status)
{
	...
	if (err < 0) {
		STEP_CDS_PR_ERR("floor_counter_get_data fail!!\n");
		return -1;
	}
	time_stamp = data.time_stamp;
...
static int step_count_recv_data(struct data_unit_t *event, void *reserved)
{
	...
	if (event->flush_action == FLUSH_ACTION)
		...
		err = step_c_data_report(event->step_counter_t.accumulated_step_count, 2);
	return err;
}
...
static int sign_recv_data(struct data_unit_t *event, void *reserved)
{
	...
	if (event->flush_action == FLUSH_ACTION)
		STEP_CDS_PR_ERR("sign do not support flush\n");
	else if (event->flush_action == DATA_ACTION)
...
static int floor_count_recv_data(struct data_unit_t *event, void *reserved)
{
	...
	if (event->flush_action == FLUSH_ACTION)
		...
		err = floor_c_data_report(event->floor_counter_t.accumulated_floor_count, 2);
	return err;
}
...
static int step_chub_local_init(void)
{
	...
	if (err) {
		STEP_CDS_PR_ERR("create attribute err = %d\n", err);
	...
	if (err) {
		STEP_CDS_PR_ERR("register step_cds control path err\n");
		goto exit;
	}
	...
	if (err) {
		STEP_CDS_PR_ERR("register step_cds data path err\n");
		goto exit;
	}
	err = scp_sensorHub_data_registration(ID_STEP_DETECTOR, step_detect_recv_data);
	if (err) {
		STEP_CDS_PR_ERR("SCP_sensorHub_data_registration fail!!\n");
		goto exit_create_attr_failed;
	}
	err =
