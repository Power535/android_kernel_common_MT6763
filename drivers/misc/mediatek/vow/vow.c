...
static struct wake_lock pcm_dump_wake_lock;
...
static void vow_service_CloseDumpFile(void)
{
	...
	wake_unlock(&pcm_dump_wake_lock);
}
...
static int vow_pcm_dump_kthread(void *data)
{
	...
	while (b_enable_dump && !kthread_should_stop()) {
		...
		case DUMP_BARGEIN: {
			...
			for (i = 0; i < (dump_package->mic_data_size / 2); i++) {
				*out_buf++ = *L_smpl++;
				*out_buf++ = *R_smpl++;
			}
		...
		case DUMP_RECOG: {
			...
			for (i = 0; i < (dump_package->recog_data_size / 2); i++) {
				*out_buf++ = *L_smpl++;
				*out_buf++ = *R_smpl++;
			}
...
static void recog_dump_routine(struct work_struct *ws)
{
	...
	dump_queue->dump_package[dump_queue->idx_w].dump_data_type =
	    DUMP_RECOG;
	dump_queue->dump_package[dump_queue->idx_w].recog_data_offset = offset;
	dump_queue->dump_package[dump_queue->idx_w].recog_data_size = data_size;
#ifdef DUALMIC_SUPPORT
	dump_queue->dump_package[dump_queue->idx_w].recog_data_offset_R = offset_R;
	dump_queue->dump_package[dump_queue->idx_w].recog_data_size_R = data_size_R;
#endif  /* #ifdef DUALMIC_SUPPORT */
...
static void vow_pcm_dump_init(void)
{
	...
	wake_lock_init(&pcm_dump_wake_lock, WAKE_LOCK_SUSPEND,
			   "pcm_dump_wake_lock");
...
#ifdef CONFIG_MTK_VOW_BARGE_IN_SUPPORT
static ssize_t VowDrv_GetBargeInDebug(struct device *kobj,
				      struct device_attribute *attr,
				      char *buf)
{
	unsigned int stat;
