...
/* hack for cur alsa */
unsigned long long rx_time_stamp;
unsigned long long tx_time_stamp;
unsigned long long rx_buf_data_equivalent_time;
unsigned long long tx_buf_data_equivalent_time;
...
static int mtk_btcvsd_snd_rx_init(struct mtk_btcvsd_snd *bt)
{
	...
	rx_time_stamp = 0;
	rx_buf_data_equivalent_time = 0;
	return 0;
}

static void get_tx_time_stamp(struct mtk_btcvsd_snd_time_buffer_info *ts)
{
	ts->time_stamp_us = tx_time_stamp;
	ts->data_count_equi_time = tx_buf_data_equivalent_time;
}

static void get_rx_time_stamp(struct mtk_btcvsd_snd_time_buffer_info *ts)
{
	ts->time_stamp_us = rx_time_stamp;
	ts->data_count_equi_time = rx_buf_data_equivalent_time;
}
			     ...
			     size_t count)
{
	 ...
	 * buf_data_equivalent_time
	 */
	tx_time_stamp = sched_clock();
	tx_buf_data_equivalent_time =
		(unsigned long long)(bt->tx->packet_w - bt->tx->packet_r) *
		packet_size * 16 * 1000 / 2 / 64;

	/* return equivalent time(us) to data count */
	tx_buf_data_equivalent_time *= 1000;
...
static int btcvsd_rx_timestamp_get(unsigned int __user *data, unsigned int size)
{
	...
	get_rx_time_stamp(&time_buffer_info_rx);

	pr_debug("%s(), time_stamp_us %llu, data_count_equi_time %llu",
		 __func__,
	...
	if (copy_to_user(data, &time_buffer_info_rx,
		...
		pr_warn("%s(), copy_to_user fail", __func__);
		ret = -EFAULT;
	}
...
static int btcvsd_tx_timestamp_get(unsigned int __user *data, unsigned int size)
{
	...
	get_tx_time_stamp(&time_buffer_info_tx);
